...
def _extract_letter_number(text: str) -> str:
    ...
    matches = re.findall(r"№\s*(\d+)", cleaned)
    for num in matches:
        if num != "250":
            return num
...
def _parse_threats(text: str) -> list[ThreatInfo]:
    ...
    for i, line in enumerate(lines):
        ...
        if not m:
            continue
        num = int(m.group(1))
        rest = stripped[m.start(2):]
        if not threat_keywords.match(rest):
            continue
        start_pos = sum(len(lines[j]) + 1 for j in range(i))
        threat_starts.append((num, start_pos, i))
...
def _extract_measures(section: str) -> list[str]:
    ...
    for m in measure_pattern.finditer(section):
        measures.append(m.group(0).strip())
    return measures[:50]
